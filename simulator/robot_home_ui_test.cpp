...
struct Surface {
  ...
  void fillScreen(int){}
  void fillTriangle(int a,int b,int c,int d,int e,int f,int){for(int v:{a,b,c,d,e,f})assert(v>=0&&v<466);}
...
int main(){
  for(int battery:{-1,0,9,78,100})for(auto mood:{robot_home::Mood::Idle,robot_home::Mood::Happy,robot_home::Mood::Sleep,robot_home::Mood::Surprise}){
    Surface s;robot_home::State state;state.batteryPercent=battery;state.mood=mood;
